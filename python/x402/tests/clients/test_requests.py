...
def test_adapter_send_success(adapter):
    ...
    with patch("requests.adapters.HTTPAdapter.send", return_value=mock_response) as mock_send:
        response = adapter.send(request)
        assert response.status_code == 200
        assert response.content == b"success"
        # Default timeout is applied when caller doesn't set one
        assert mock_send.call_args.kwargs.get("timeout") == 60.0
