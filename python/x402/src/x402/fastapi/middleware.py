...
def require_payment(
    ...
    facilitator_timeout_seconds: float = 60.0,
    ...
    Args:
        ...
        facilitator_timeout_seconds (float, optional): Default HTTP timeout (seconds) when calling the facilitator
            (verify/settle/list). If `facilitator_config` already includes a `timeout`, that value wins.
    ...
    # Ensure facilitator calls do not hang indefinitely (default 60s).
    # Important: don't pass an empty config dict, or FacilitatorClient will treat url as empty.
    if facilitator_config is None:
        facilitator = FacilitatorClient({"timeout": facilitator_timeout_seconds})
    else:
        cfg: FacilitatorConfig = dict(facilitator_config)
        cfg.setdefault("timeout", facilitator_timeout_seconds)
        facilitator = FacilitatorClient(cfg)
