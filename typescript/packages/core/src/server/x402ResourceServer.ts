...
export class x402ResourceServer {
  ...
  async buildPaymentRequirements(resourceConfig: ResourceConfig): Promise<PaymentRequirements[]> {
    ...
    // Derive networkId from `Network` format `${string}:${string}` (e.g. "EIP155:1" -> "1")
    // We split on the first ":" to preserve any ":" that might appear in the right-hand side.
    const networkStr = resourceConfig.network as unknown as string;
    const colonIndex = networkStr.indexOf(":");
    const networkId = colonIndex === -1 ? "" : networkStr.slice(colonIndex + 1);
