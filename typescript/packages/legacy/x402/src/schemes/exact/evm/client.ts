...
import {createNonce, signAuthorization, signAuthorizationNoSuperEip3009} from "./sign";
...
export async function signPaymentHeader<transport extends Transport, chain extends Chain>(
  ...
  const supportsTransferWithAuthorization = paymentRequirements.asset
      ? await verifyTransferWithAuthorizationSupport(client, paymentRequirements.asset)
      : false;
  ...
  if (supportsTransferWithAuthorization) {
    ...
    authorizationResult = await signAuthorizationNoSuperEip3009(
        client,
        unsignedPaymentHeader.payload.authorization,
        paymentRequirements,
    );
