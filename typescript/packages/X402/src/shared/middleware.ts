...
export function findMatchingPaymentRequirements(
  ...
  // First try to find exact match including amount and recipient
  const exactMatch = paymentRequirements.find((requirement) => {
    const schemeMatch = requirement.scheme === payment.scheme;
    const networkMatch = requirement.networkId === payment.networkId;

    console.log(`[DEBUG] Checking requirement ${requirement.tokenSymbol}:`, {
      schemeMatch,
      networkMatch,
      requirementAmount: requirement.amountRequired,
      requirementPayTo: requirement.payToAddress
    });

    // For authorization payments, also check amount and recipient
    if (payment.payload?.authorization) {
      const auth = payment.payload.authorization;

      // Convert requirement amount to wei if it's in human readable format
      let requirementAmountInWei = requirement.amountRequired;
      if (requirement.amountRequiredFormat === "humanReadable" && requirement.tokenDecimals) {
        requirementAmountInWei = Number(requirement.amountRequired) * Math.pow(10, requirement.tokenDecimals);
      }

      const amountMatch = requirementAmountInWei === Number(auth.value) ||
                         requirement.amountRequired === Number(auth.value) ||
                         BigInt(requirementAmountInWei) === auth.value;
      const recipientMatch = requirement.payToAddress?.toLowerCase() === auth.to?.toLowerCase();

      console.log(`[DEBUG] Authorization check for ${requirement.tokenSymbol}:`, {
        authValue: auth.value,
        authValueAsNumber: Number(auth.value),
        requirementAmount: requirement.amountRequired,
        requirementAmountInWei,
        amountMatch,
        authTo: auth.to,
        requirementPayTo: requirement.payToAddress,
        recipientMatch,
        finalMatch: schemeMatch && networkMatch && amountMatch && recipientMatch
      });

      return schemeMatch && networkMatch && amountMatch && recipientMatch;
    }
...
export function createRouteConfigFromPrice(
  ...
  const paymentRequirements: PaymentRequirements = {
    ...
    payToAddress: evmAddress || "0x0000000000000000000000000000000000000000" as any,
    tokenAddress: asset.address as any,
    tokenSymbol: network === "bsc"  || network === "polygon" || network === "sei" ? "USDT" : "USDC",
...
export function getUsdcAddressForChain(chainId: number | string): string {
  ...
  if (chainId === 8453) {
    return STABLECOIN_ADDRESSES.USDC_BASE;
  }
  if (chainId === 137) {
